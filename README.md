...
*(Здесь можно будет добавить результаты для ViLBERT)*
...
### Гипотеза 4: Context Aware рекомендации

Почти сразу выкинул эту идею, в социальных сетях у нас много мемов, одинаковых картинок, паст и тд. Часто текст и картинки даже не соотвествуют друг-другу. По факту найдя похожести мы вряд-ли получим более точные рекомендации чем у того-же catboost`а. Но я бы попробовал построить какие-нибудь als_emb это могла бы быть сильная фича, которая закладывает в себе много коллаборативного сигнала. Ну или веса, ембеды похожих айтемов, средний эмбеддинг. Можно сегментировать эти эмбеддинги и тд. Куда сильнее сработали статистики, но к сожалению инфы по ним у нас нет.
...
* Эксперименты с другими моделями: Попробовать другие архитектуры мультимодальных моделей или методы ансамблирования. (Например Sbert, другие модели CLIP, дообучать CLIP и тд)

* Добавить аугментации и лучшую предобработку картинок и текстов, у нас могут быть фичи по мусорным постам с кучей хештегов, OCR текстов на картинках, всякие рекламные картинки и посты по типу "скидки на вайлдберис", а так-же детекций и размеров всяких объектов, лиц, эмоций. Хочется так-же фичи по кликбейтам.

* Улучшить негативное семплировать, сделать всякие logQ коррекции что бы сбалансировать выбросы и тд 
